...
def session_fixture():
    ...
    # Patch the database engine BEFORE any operations
    old_engine_getter = database.get_engine
    old_engine_proxy = database.engine
    
    # Override get_engine to return test engine
    def mock_get_engine():
        return test_engine
    
    database.get_engine = mock_get_engine
    database._engine = test_engine  # Direct assignment for lazy init
    ...
    database.get_engine = old_engine_getter
    database._engine = None  # Reset lazy init
...
def test_read_tasks_filter(manager):
    """Test reading tasks with priority filter."""
...
def test_create_task_idempotency(manager):
    """Test that duplicate requests with same client_request_id return existing task."""
...
def test_update_task(manager):
    ...
    updated = manager.update_task(UpdateTaskInput(task_id=str(task.id), title="New Title", completed=True))

    assert updated.title == "New Title"
    assert updated.completed is True

    stored = manager.get_task(str(task.id))
...
def test_delete_task(manager):
    ...
    assert manager.delete_task(str(task.id)) is True
    assert manager.get_task(str(task.id)) is None
